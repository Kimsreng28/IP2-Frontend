...
import { Bookmark, ChevronLeft, ChevronRight, ChevronRightIcon, Minus, Plus, Star, ThumbsUp } from "lucide-react";
import Image from "next/image";
import { useEffect, useState } from "react";

export default function ProductPage() {
  const [activeTab, setActiveTab] = useState<'ratings' | 'questions' | 'reviews'>('ratings');
  ...
  return (
    <>
      <div className="w-full bg-gray-50">
        <div className="flex items-start justify-start w-full p-5 py-8 mx-auto max-w-7xl mt-30">
          <div className="flex items-start space-x-2 text-sm font-medium text-gray-700">
            <span className="text-black cursor-pointer hover:text-blue-600">Home</span>
              ...
              <div className="flex space-x-4">
                {images.map((image, index) => (
                  <button
                    ...
                    className={`w-20 h-20 bg-white rounded-lg overflow-hidden border-2 ${selectedImage === index ? "border-gray-900" : "border-gray-200"
                      }`}
                  >
                    <Image
                ...
                <div className="flex space-x-3">
                  {colors.map((color, index) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-lg border-2 ${selectedColor === index ? "border-gray-900" : "border-gray-200"
                        } ${color.value}`}
                    />
                  ))}
                </div>
        ...
        <div className="px-6 py-8 mx-auto max-w-7xl">
          ...
          <div className="flex border-b border-gray-200">
            {['ratings', 'questions', 'reviews'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab as 'ratings' | 'questions' | 'reviews')}
                className={`px-4 py-2 font-medium ${activeTab === tab ? 'text-gray-900 border-b-2 border-gray-900' : 'text-gray-500 hover:text-gray-700'
                  }`}
              >
            ...
            <>
                  ...
                  <div className="ml-4">
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-5 h-5 ${i < 4 ? 'text-black fill-black' : 'text-gray-300'}`}
                        />
                      ))}
                    </div>
              ...
              <div className="pb-8 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900">Sofia Harvetz</h3>
            ...
            <div className="py-6 border-b border-gray-200">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-gray-600">Does this product come with a warranty?</p>
          ...
          {activeTab === 'reviews' && (
            <div className="py-6">
              ...
              <p className="mt-2 text-gray-600">
                The quality exceeded my expectations. Very durable and looks exactly like the pictures.
              </p>
              <div className="flex items-center mt-4 space-x-4 text-sm">
                <button className="flex items-center text-gray-500 hover:text-gray-700">
                  <ThumbsUp className="w-4 h-4 mr-1" />
